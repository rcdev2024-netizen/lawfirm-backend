...
def list_users_with_roles(
    ...
    query = (
        supabase.table("users")
        .select("id, full_name, email, role_id, phone, is_active, created_at, roles!users_role_id_fkey(name)")
    )
    if role:
        role_lookup = supabase.table("roles").select("id").eq("name", role).execute()
        if role_lookup.data:
            query = query.eq("role_id", role_lookup.data[0]["id"])
    result = query.order("created_at", desc=True).execute()
    rows = result.data or []
    for row in rows:
        auth_utils._normalize_user(row)
    return rows
...
def get_user_role(user_id: int, admin: dict = Depends(require_admin)):
    ...
    return auth_utils._normalize_user(result.data[0])
...
def update_user_role(
    ...
    user_result = (
        supabase.table("users")
        .select("id, role_id, full_name, email, phone, is_active, created_at, roles!users_role_id_fkey(name)")
        .eq("id", user_id)
        .execute()
    )
    ...
    user = auth_utils._normalize_user(user_result.data[0])
    old_role_id = user["role_id"]
    old_role_name = user.get("role", "client")
    ...
    if not new_role_result.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Role '{payload.role}' does not exist or is inactive"
        )

    new_role = new_role_result.data[0]

    if old_role_id == new_role["id"]:
        return user
    ...
    updated = auth_utils._normalize_user(update_result.data[0])
    if "role" not in updated:
        updated["role"] = new_role["name"]
    return updated
